...
public class MessageProcessor : IMessageProcessor
{
    ...
    public async Task<IChatState?> ExecuteWithMiddlewares(Message message, IChatState? state, IChatState? oldState)
    {
        ...
        if (!_middlewares.Any()) return await Process(executingState, message);
        ...
        {
            if (!context.ExecutingState.Equals(oldState))
                await _stateStore.NotifyReplacedStates(message.ChatId, oldState, context.ExecutingState);
            IChatState? nextState = await Process(context.ExecutingState, context.Message);
            if (!context.ExecutingState.Equals(nextState))
                await _stateStore.NotifyReplacedStates(message.ChatId, context.ExecutingState, nextState);
            return nextState;
        });
